<template>
  <b-modal id="myModal" title="Новый заказ" @hidden="onHidden" hide-footer visible static>
    <b-form @submit.prevent="onSubmit">
      <b-form-group
        label="Выберите покупателя:"
        label-for="input-user">
        <pick-user id="input-user" v-model="record.user_id" :disabled="this.loading" :role_id="[3]" required/>
      </b-form-group>
      ...
      <b-form-group
        ...
        <b-form-select v-model="record.period" :disabled="this.loading || !Object.keys(price).length">
          <option v-for="(cost, period) in price" :value="period">
...
<script>
    export default {
        ...
        asyncData({app}) {
            return app.$axios.get('admin/courses')
                .then(res => {
        ...
        methods: {
            ...
            onSubmit() {
                this.loading = true;
                this.$axios.put('admin/orders', this.record)
                    .then(res => {
                        this.loading = false;
