    ...
    <b-modal id="myNestedModal" :title="record.title" @hidden="onHidden" hide-footer visible static size="lg">
      <b-form @submit.prevent="onSubmit">
        ...
        <b-row no-gutters class="mt-4 justify-content-between">
          <b-button variant="secondary" @click="$root.$emit('bv::hide::modal', 'myNestedModal')" :disabled="this.loading">Отмена
          </b-button>
          <b-button variant="primary" type="submit" :disabled="this.loading">
            <b-spinner small v-if="loading"/> Сохранить
          </b-button>
        </b-row>
      </b-form>
    </b-modal>
...
<script>
    ...
    export default {
        ...
        methods: {
            ...
            onSubmit() {
                this.loading = true;
                this.$axios.patch('admin/lessons', this.record)
                    .then(res => {
                        this.loading = false;
