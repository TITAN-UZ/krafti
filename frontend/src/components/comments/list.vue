<template>
  <div class="lesson-comments">
    ...
    <div class="comments__content">
      <!-- Form -->
      <comments-form v-if="!readOnly && $auth.loggedIn && parentId == 0" v-model="text" :show-cancel="false" @onSubmit="onSubmit" />
          ...
          <div v-if="!readOnly && isUser && parentId !== child.id" class="d-flex align-items-center mt-1">
            ...
            <div v-if="isAdmin" class="d-flex ml-auto align-items-center">
              <div v-if="!child.deleted" class="mr-2">
                <fa v-if="child.review === false" :icon="['fal', 'star']" class="text-success" @click="onReview(child)" />
...
export default {
  ...
  methods: {
    async loadComments() {
      try {
        const {data: res} = await this.$axios.get(this.url, {params: {lesson_id: this.lessonId, course_id: this.courseId, limit: 1000}})
    ...
    async onSubmit() {
      ...
      try {
        await this.$axios.put(this.url, {course_id: this.courseId, lesson_id: this.lessonId, parent_id: this.parentId, text: this.text})
        this.onCancel()
        this.loadComments()
      } catch (e) {
        console.error(e)
    ...
    async onDelete(item) {
      item.deleted = true
      await this.$axios.patch(this.url, {course_id: this.courseId, lesson_id: this.lessonId, id: item.id, deleted: true})
    },
    async onRestore(item) {
      item.deleted = false
      await this.$axios.patch(this.url, {course_id: this.courseId, lesson_id: this.lessonId, id: item.id, deleted: false})
    },
    async onReview(item) {
      item.review = true
    ...
    async unReview(item) {
      item.review = false
      await this.$axios.patch(this.url, {course_id: this.courseId, lesson_id: this.lessonId, id: item.id, review: false})
    },
  },
}
</script>
