      ...
      <div v-for="review in reviews" :key="review.id" :class="itemClass">
        <div class="review__item d-flex flex-column justify-content-center align-items-center">
          <div class="review__item--photo">
            <img v-if="review.user.photo" class="rounded-circle" :src="review.user.photo" />
            <fa v-else :icon="['fad', 'user-circle']" style="--fa-primary-color: #fff" class="user-placeholder" />
          </div>
          <h2 class="review__item--name">{{ review.user.fullname }}</h2>
...
export default {
  ...
  props: {
    ...
    itemClass: {
      type: String,
      required: false,
      default: 'col-12 col-lg-4 m-width-80',
    },
  },
  created() {
    this.$fa.add(faUserCircle)
  },
}
</script>
