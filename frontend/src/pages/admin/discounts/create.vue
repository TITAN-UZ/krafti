<template>
  <b-modal id="myModal" title="Новый промокод" hide-footer visible static @hidden="onHidden">
    <b-form @submit.prevent="onSubmit">
      <b-form-group label="Промокод:" label-for="input-code" description="Код должен быть уникален">
        <b-form-input id="input-code" v-model="record.code" autofocus required />
      </b-form-group>

      <b-form-group label="Скидка:" label-for="input-discount" description="">
        <b-input-group :append="record.percent ? '%' : 'руб.'" class="mb-2">
          <b-form-input id="input-discount" v-model="record.discount" type="number" required />
        </b-input-group>
      ...
      <b-form-group label="Количество использований:" label-for="input-limit" description="">
